...
namespace chainsql::kv {
    ...
    namespace detail {
        ...
        inline uint64_t itr_create(std::string_view prefix)
        {
            return internal_use_do_not_use::kv_it_create(prefix.data(), prefix.size());
        }

        inline void itr_destroy(uint64_t itr) { internal_use_do_not_use::kv_it_destroy(itr); }
        inline int32_t itr_status(uint64_t itr) { return internal_use_do_not_use::kv_it_status(itr); }
         ...
         *
         * Note that this type in `chainsql::kv::map` is defined as `chainsql::kv::map::elem_t`.
         */
        ...
        struct elem
        {
            ...
            elem() = default;
            constexpr inline elem(const key_t &k, value_t v)
                : key(KV::full_key(k)), value(std::move(v)) {}

            inline elem(key_type k, value_t v, packed_tag)
         ...
         * @ingroup keyvaluemap
         *
         * @brief This struct represents the iterator for the `chainsql::kv::map` data type.
         ...
         *
         * Note that this iterator type in `chainsql::kv::map` is defined as `chainsql::kv::map::iterator_t`.
         * There is also a reverse iterator available, it is defined as `chainsql::kv::map::reverse_iterator_t`.
        ...
        struct iterator
        {
            ...
            using value_t = typename KV::value_t;
            constexpr static inline uint64_t invalidated_iterator = std::numeric_limits<uint64_t>::max();
            ...
            inline iterator()
                : element(),
                  handle(itr_create({KV::prefix().data(), KV::prefix().size()})) {}
    ...
    } // namespace chainsql::kv::detail

    /**
     * @ingroup keyvaluemap
     *
     * @brief Defines an Chainsql Key Value Map
     * @details Chainsql Key Value API provides a C++ interface to the Chainsql Key Value database.
     ...
     */
    template <chainsql::name::raw TableName, typename K, typename V, chainsql::name::raw IndexName = chainsql::name("map.index")>
    class map
    {
    public:
        ...
        {
            static key_type prfx = chainsql::detail::const_pack(magic, table_name, index_name);
            return prfx;
        }
        ...
        {
            writable_wrapper(key_type k, value_t v)
                : element(std::move(k), std::move(v), packed_tag{}) {}
            ...
            {
                map{}.set(element.key, o, packed_tag{});
                element.value = o;
                return *this;
            }
        ...
        writable_wrapper operator[](const key_t &k)
        {
            ...
            set(std::get<1>(v), value_t{}, packed_tag{});
            return {std::move(std::get<1>(v)), value_t{}};
        }

        template <typename Key>
        writable_wrapper at(Key &&k)
        {
        ...
        bool raw_write(const key_t &k, std::string_view bytes) const
        {
            ...
            auto wrote = kv_set(fk.data(), fk.size(), bytes.data(), bytes.size());
    ...
    protected:
        ...
        writable_wrapper at(const key_type &bytes)
        {
        ...
        inline bool set(const key_type &k, const value_t &v, packed_tag) const
        {
            using namespace internal_use_do_not_use;
            const auto &packed_value = pack_value(v);
            auto wrote = kv_set(k.data(), k.size(), packed_value.data(), packed_value.size());
            return wrote == packed_value.size();
        }

        inline bool set(const key_t &k, const value_t &v) const
        {
            return set(full_key(k), v, packed_tag{});
        }
