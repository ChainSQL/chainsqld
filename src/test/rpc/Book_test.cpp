...
public:
    ...
    testOneSideEmptyBook()
    {
...

        {
            ...
            books[jss::books] = Json::arrayValue;
            {
                ...
                j[jss::snapshot] = true;
                j[jss::taker_gets][jss::currency] = "ZXC";
...

        {
            ...
            env(offer("alice", ZXC(700), USD(100)),
            ...
            BEAST_EXPECT(wsc->findMsg(5s, [&](auto const& jv) {
                ...
                return t[jss::TransactionType] == jss::OfferCreate &&
                    ...
                    t[jss::TakerPays] ==
                    ZXC(700).value().getJson(JsonOptions::none);
            }));
        }

        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), ZXC(75)), require(owners("alice", 2)));
            env.close();
    ...
    testOneSideOffersInBook()
    {
        ...
        env(offer("alice", ZXC(500), USD(100)), require(owners("alice", 1)));

        // Create a bid: TakerPays 100/USD, TakerGets 200
        env(offer("alice", USD(100), ZXC(200)), require(owners("alice", 2)));
        env.close();

        {
            ...
            BEAST_EXPECT(
                ...
                ZXC(200).value().getJson(JsonOptions::none));
            BEAST_EXPECT(
        ...
        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), ZXC(75)), require(owners("alice", 4)));
            env.close();
            BEAST_EXPECT(!wsc->getMsg(10ms));
        }
    ...
    testBothSidesEmptyBook()
    {
...

        {
            ...
            BEAST_EXPECT(wsc->findMsg(5s, [&](auto const& jv) {
                ...
                return t[jss::TransactionType] == jss::OfferCreate &&
                    t[jss::TakerGets] ==
                    ZXC(75).value().getJson(JsonOptions::none) &&
                    t[jss::TakerPays] ==
                    USD(100).value().getJson(JsonOptions::none);
            }));
        }
    ...
    testBothSidesOffersInBook()
    {
...

        {
            ...
            BEAST_EXPECT(
                ...
                ZXC(500).value().getJson(JsonOptions::none));
            BEAST_EXPECT(
    ...
    testMultipleBooksOneSideOffersInBook()
    {
        ...
        // Create a bid: TakerPays 100/USD, TakerGets 200
        env(offer("alice", USD(100), ZXC(200)), require(owners("alice", 3)));
        ...
        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), ZXC(75)), require(owners("alice", 6)));
            env.close();
            BEAST_EXPECT(!wsc->getMsg(10ms));
        }
    ...
    testTrackOffers()
    {
        ...
        Account bob{"bob"};
        auto wsc = makeWSClient(env.app().config());
        env.fund(ZXC(20000), alice, bob, gw);
        env.close();
        auto USD = gw["USD"];
        ...
        env(offer(alice, ZXC(4000), USD(10)));
        env.close();

        Json::Value jvParams;
        jvParams[jss::taker] = env.master.human();
        jvParams[jss::taker_pays][jss::currency] = "ZXC";
        ...
        BEAST_EXPECT(
            ...
            ZXC(4000).value().getJson(JsonOptions::none));
        ...
        BEAST_EXPECT(wsc->findMsg(5s, [&](auto const& jval) {
            ...
            return t[jss::TransactionType] == jss::OfferCreate &&
                ...
                t[jss::TakerPays] ==
                ZXC(2000).value().getJson(JsonOptions::none);
        }));
    ...
    testCrossingSingleBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice and Bob place identical offers for USD -> ZXC
        ...
        env.fund(ZXC(1000000), gw, alice, bob, charlie);
        env.close();
        ...
        // Alice and Bob offer $500 for 500 ZXC
        env(offer(alice, ZXC(500), USD(500)));
        env(offer(bob, ZXC(500), USD(500)));
        env.close();
        ...
        env(offer(charlie, USD(1000), ZXC(1000)));
        env.close();
        env.require(offers(alice, 0), offers(bob, 0), offers(charlie, 0));
        using namespace std::chrono_literals;
        BEAST_EXPECT(offerOnlyOnceInStream(wsc, 1s, ZXC(1000), USD(1000)));
    ...
    testCrossingMultiBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice has 1 USD and wants 100 ZXC
        //  - Bob has 100 ZXC and wants 1 EUR
        ...
        env(offer(alice, ZXC(100), USD(1)));
        env(offer(bob, EUR(1), ZXC(100)));
        env.close();
...

        {
...

            {
                ...
                j[jss::taker_pays][jss::currency] = "ZXC";
            }
    ...
    testBookOfferErrors()
    {
...

        {
            ...
            jvParams[jss::taker_gets][jss::currency] = "ZXC";
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams))[jss::result];
...

        {
            ...
            BEAST_EXPECT(
                ...
                "for ZXC currency specification.");
        }

        {
            ...
            jvParams[jss::taker_pays][jss::issuer] = toBase58(zxcAccount());
            jvParams[jss::taker_gets][jss::currency] = "USD";
            jvParams[jss::taker_gets][jss::issuer] = gw.human();
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams))[jss::result];
    ...
    testBookOfferLimits(bool asAdmin)
    {
        ...
        Account gw{"gw"};
        env.fund(ZXC(200000), gw);
        env.close();
        auto USD = gw["USD"];

        for (auto i = 0; i <= RPC::Tuning::bookOffers.rmax; i++)
            env(offer(gw, ZXC(50 + 1 * i), USD(1.0 + 0.1 * i)));
        env.close();
