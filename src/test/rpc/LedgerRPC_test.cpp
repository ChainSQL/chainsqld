...
class LedgerRPC_test : public beast::unit_test::suite
{
    ...
    void testLedgerEntryCheck()
    {
        ...
        env (checkCreate (env.master, alice, ZXC(100)));
        env.close();
    ...
    void testLedgerEntryEscrow()
    {
        ...
        env (escrowCreate (alice, alice, ZXC(333), env.now() + 2s));
        env.close();
        ...
        std::string escrowIndex;
        {
            ...
            Json::Value const jrr = env.rpc (
                "json", "ledger_entry", to_string (jvParams))[jss::result];
            BEAST_EXPECT(
                jrr[jss::node][jss::Amount] == ZXC(333).value().getText());
    ...
    void testLedgerEntryOffer()
    {
        ...
        env (offer (alice, USD (321), ZXC (322)));
        env.close();
    ...
    void testLedgerEntryPayChan()
    {
        ...
        env (payChanCreate (alice, env.master, ZXC(57), 18s, alice.pk()));
        env.close();
    ...
    void testLookupLedger()
    {
        ...
        env.close();
        env.fund(ZXC(10000), "jim");
        env.close();
        env.fund(ZXC(10000), "jill");
    ...
    void testQueue()
    {
        ...
        env.fund(ZXC(10000), bob);
        env.close();
        env.fund(ZXC(10000), charlie);
        env.fund(ZXC(10000), daria);
        env.close();
        ...
        env(pay(alice, "george", ZXC(1000)), json(R"({"LastLedgerSequence":7})"),
            ter(terQUEUED));
        env(offer(alice, ZXC(50000), alice["USD"](5000)), seq(aliceSeq + 1),
            ter(terQUEUED));
