	...
	namespace test {
		...
		class Executive_test : public beast::unit_test::suite {
			...
			std::pair<AccountID, bool> executeCreate(Account sender, Blob data)
			{
				...
				SleOps ops(ctx);
				auto pInfo = std::make_shared<EnvInfoImpl>(1, 210000,1000,0,0,false, ctx.app.getPreContractFace());
			...
			bool executeCall(Account sender, AccountID &contract, Blob data)
			{
				auto& ctx = getApplyContext();
				SleOps ops(ctx);
				auto pInfo = std::make_shared<EnvInfoImpl>(pCtx_->view().info().seq, 210000,1000,0,0,false, ctx.app.getPreContractFace());
				Executive e(ops, *pInfo, 1);
