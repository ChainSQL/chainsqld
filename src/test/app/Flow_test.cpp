...
struct Flow_test : public beast::unit_test::suite
{
    ...
    testBookStep(FeatureBitset features)
    {
        ...
        }
        {
            ...
            env(pay(alice, carol, USD(50)), path(~ZXC, ~USD), sendmax(BTC(50)));
        ...
        }
        {
            ...
            env(pay(alice, carol, ZXC(50)), path(~ZXC), sendmax(USD(50)));
    ...
    testTransferRate(FeatureBitset features)
    {
...

        {
            ...
            env(pay(alice, bob, USD(100)), sendmax(ZXC(100)));
            env.require(
        ...
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(pay(alice, bob, USD(100)), sendmax(ZXC(200)));
            env.require(
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        // Computing the incoming zxc to the ZXC/USD offer will require two
    ...
    void
    testSelfFundedZXCEndpoint(bool consumeOffer, FeatureBitset features)
    {
    ...
    testRIPD1449()
    {
        ...
        // pay alice -> zxc -> USD/bob -> bob -> gw -> alice
        ...
        env(offer(carol, ZXC(1), bob["USD"](1000)));
        env.close();
    ...
    testSelfPayLowQualityOffer(FeatureBitset features)
    {
        ...
        // ZXC than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    testZXCPathLoop()
    {
        ...
        for (auto const withFix : {true, false})
        {
            ...
            {
                // Payment path starting with ZXC
                Env env(*this, feats);
                env.fund(ZXC(10000), alice, bob, gw);
                env.trust(USD(1000), alice, bob);
                ...
                env(offer(alice, ZXC(100), USD(100)), txflags(tfPassive));
                env(offer(alice, USD(100), ZXC(100)), txflags(tfPassive));
                env(offer(alice, ZXC(100), EUR(100)), txflags(tfPassive));
                env.close();
        ...
        }
        {
            ...
            env(offer(alice, ZXC(100), USD(100)), txflags(tfPassive));
            env(offer(alice, EUR(100), ZXC(100)), txflags(tfPassive));
            env.close();
            // EUR -> //ZXC -> //USD ->ZXC
            env(pay(alice, bob, ZXC(1)),
                path(~ZXC, ~USD, ~ZXC),
                sendmax(EUR(1)),
                txflags(tfNoRippleDirect),
        ...
        }
        {
            ...
            env(offer(alice, ZXC(100), EUR(100)), txflags(tfPassive));
            env(offer(alice, EUR(100), ZXC(100)), txflags(tfPassive));
            env(offer(alice, ZXC(100), JPY(100)), txflags(tfPassive));
            env.close();

            env(pay(alice, bob, JPY(1)),
                path(~ZXC, ~EUR, ~ZXC, ~JPY),
                sendmax(USD(1)),
                txflags(tfNoRippleDirect),
                ter(temBAD_PATH_LOOP));
        }
    ...
    testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedZXCEndpoint(false, features);
        testSelfFundedZXCEndpoint(true, features);
