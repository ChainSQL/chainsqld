...
bool equal (std::unique_ptr<Step> const& s1,
            ZXCEndpointStepInfo const& zxcsi)
{
    if (!s1)
        return false;
...
struct Flow_test : public beast::unit_test::suite
{
    ...
    static auto IAPE(AccountID const& account)
    {
        return STPathElement (
            ...
            zxcAccount (), zxcCurrency (), account);
    };
    ...
    void testToStrand ()
    {
...

        {
            ...
            test (env, USD, zxcIssue (), STPath ({IPE (USD)}), tesSUCCESS,
                ZXCS{alice}, B{ZXC, USD}, D{gw, bob, usdC});

            // Path with ZXC dst currency
            test (env, zxcIssue(), USD.issue (), STPath ({IPE (ZXC)}),
                  tesSUCCESS, D{alice, gw, usdC}, B{USD, ZXC}, ZXCS{bob});

            // Path with ZXC cross currency bridged payment
            test (env, EUR, USD.issue (), STPath ({CPE (zxcCurrency ())}),
                  tesSUCCESS,
                  D{alice, gw, usdC}, B{USD, ZXC}, B{ZXC, EUR}, D{gw, bob, eurC});
...

            {
                ...
                auto flowJournal = env.app ().logs ().journal ("Flow");
                {
                    ...
                    auto r = toStrand (*env.current (), alice,
                        zxcAccount (), ZXC, USD.issue (), STPath (), true, flowJournal);
                    BEAST_EXPECT(r.first == temBAD_PATH);
                }
                {
                    // The root account can't be the src
                    auto r =
                        toStrand (*env.current (), zxcAccount (),
                            alice, ZXC, boost::none, STPath (), true, flowJournal);
                    BEAST_EXPECT(r.first == temBAD_PATH);
                }
                {
                    // The root account can't be the src
...

        {
            ...
            env (pay (alice, carol, USD (100)), path (~USD, ~EUR, ~USD),
                sendmax (ZXC (200)), txflags (tfNoRippleDirect),
    ...
    void testDirectStep ()
    {
        ...
        }
        {
            ...
            env (pay (alice, bob, ZXC (100)));
            env.require (balance (bob, ZXC (10000 + 100)));
            env.require (balance (alice, zxcMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, features(featureFlow), features(featureOwnerPaysFee));
        ...
        }
        {
            ...
            env.fund (ZXC (10000), alice, bob, carol, dan, erin);
    ...
    void testBookStep ()
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), ZXC (50)));
            env (offer (bob, ZXC (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~ZXC, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, ZXC (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), ZXC (50)));
        }
        {
            // simple ZXC -> USD through offer and sendmax
            Env env (*this, features(featureFlow), features(featureOwnerPaysFee));
            ...
            env.require (balance (bob, zxcMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, ZXC (50), USD (50)));
        }
        {
            // simple USD -> ZXC through offer and sendmax
            Env env (*this, features(featureFlow), features(featureOwnerPaysFee));
            ...
            env (offer (bob, USD (50), ZXC (50)));

            env (pay (alice, carol, ZXC (50)), path (~ZXC),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, zxcMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, ZXC (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), ZXC (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~ZXC, ~EUR),
    ...
    void testTransferRate ()
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (ZXC (50)));
            env.require (
                balance (alice, zxcMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (ZXC (40)));
            env.require (
                balance (alice,  zxcMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, ZXC (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (ZXC (100)));
            env.require (
                balance (alice, zxcMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, ZXC (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (ZXC (200)));
            env.require (
                balance (alice, zxcMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDryHelper (jtx::Env& env)
    {
        ...
        env.fund (ZXC(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 zxc available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming zxc to the ZXC/USD offer will require two
        // recursive calls to the EUR/ZXC offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), ZXC (50)));
        env (offer (bob, ZXC (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~ZXC, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for(auto const& d: {-timeDelta*100, +timeDelta*100}){
            ...
            env (pay (gw, bob, USD (100)));
            env (offer (bob, ZXC (50), USD (50)));
            env (offer (bob, ZXC (100), USD (50)));

            auto expectedResult =
                closeTime < amendmentRIPD1141SoTime () ? tecPATH_DRY : tesSUCCESS;
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (ZXC (100)),
    ...
    static ZXCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedZXCEndpoint (bool consumeOffer)
    {
        // Test that the deferred credit table is not bypassed for
        // ZXCEndpointSteps. If the account in the first step is sending ZXC and
        // that account also owns an offer that receives ZXC, it should not be
        // possible for that step to use the ZXC received in the offer as part
        // of the payment.
        testcase("Self funded ZXCEndpoint");
        ...
        env(offer(alice, ZXC(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(ZXC(20000)),
    ...
    void testUnfundedOffer (bool withFix)
    {
        ...
        using namespace jtx;
        {
            ...
            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, ZXC(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, ZXC(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~ZXC),
                sendmax(USD(1)), txflags(tfNoRippleDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), ZXC(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), ZXC(0)));
        }
    }
    ...
    testReexecuteDirectStep(Features&&... fs)
    {
        ...
        env(pay(alice, bob, ZXC(10000)), path(~ZXC), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoRippleDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(ZXC(100000000), alice, noripple(bob), carol, gw);
        ...
        env(offer(alice, bob["USD"](1000), ZXC(1)));
        env.close();

        env(pay(alice, alice, ZXC(1)), path(gw, bob, ~ZXC),
        ...
        env(offer(alice, ZXC(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(ZXC(100000)), txflags(tfNoRippleDirect),
            ter(withFix ? tecPATH_DRY : tesSUCCESS));
        env.close();
    ...
    void run() override
    {
        ...
        testSelfPayment1();
        testSelfPayment2();
        testSelfFundedZXCEndpoint(false);
        testSelfFundedZXCEndpoint(true);
