...
class AccountTxPaging_test : public beast::unit_test::suite
{
    ...
    testAccountTxPaging()
    {
        ...
        for (auto i = 0; i < 5; ++i)
        {
            ...
            env(offer(A1, ZXC(11), A1["USD"](1)));
            env(offer(A2, ZXC(10), A2["USD"](1)));
            env(offer(A3, ZXC(9), A3["USD"](1)));
            env.close();
        }
    ...
    testAccountTxContentsGrpc()
    {
        ...
        // Payment
        env(pay(alice, gw, ZXC(100)), stag(42), dtag(24), last_ledger_seq(20));
        ...
        env(offer(alice, USD(50), ZXC(150)), sig(alie));
        ...
        // Escrow
        {
            ...
            Json::Value escrowWithFinish{escrow(alice, alice, ZXC(500))};
            ...
            txns.emplace_back(env.tx());
            Json::Value escrowWithCancel{escrow(alice, alice, ZXC(500))};
        ...
        // Check
        {
            ...
            env(check::create(alice, gw, ZXC(300)), sig(alie));

            auto txn = env.tx();
            auto const gwCheckId = keylet::check(gw, env.seq(gw)).key;
            env(check::create(gw, alice, ZXC(200)));
            env.close();
            ...
            env(check::cash(alice, gwCheckId, ZXC(200)), sig(alie));
            ...
            {5,
             ...
             [&txns, this](auto res) {
                 ...
                 return BEAST_EXPECT(res.has_payment()) &&
                     BEAST_EXPECT(
                            res.payment().amount().value().has_zxc_amount()) &&
                     BEAST_EXPECT(
                            res.payment()
                                .amount()
                                .value()
