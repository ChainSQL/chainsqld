...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct ZXC payments to an account
        ...
        env (offer (carol, USD(100), ZXC(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            // Note that even though alice is paying bob in ZXC, the payment
        ...
        //  Test when bob has an ZXC balance > base reserve.
        failedIouPayments();

        // Set bob's ZXC balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysZXC {
                env.balance (bob, ZXC) - reserve (env, 1)};
            ZXCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysZXC), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's ZXC balance == base reserve.
        BEAST_EXPECT (env.balance (bob, ZXC) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an ZXC balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, ZXC) == ZXC (0));
        failedIouPayments();

        // Give bob enough ZXC for the fee to clear the lsfDepositAuth flag.
    ...
    void testPayZXC()
    {
        // Exercise direct ZXC payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, ZXC) == ZXC (10000) - drops(10));

        // bob has more ZXC than the base reserve.  Any ZXC payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, ZXC) == ZXC (10000) - drops(10));

        // Change bob's ZXC balance to exactly the base reserve.
        {
            STAmount const bobPaysZXC {
                env.balance (bob, ZXC) - reserve (env, 1)};
            ZXCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct ZXC
        ...
        BEAST_EXPECT (env.balance (bob, ZXC) == reserve (env, 0) + drops(1));
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 ZXC.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, ZXC) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, ZXC) ==
        ...
        // Take bob back down to a zero ZXC balance.
        env (noop (bob), fee (env.balance (bob, ZXC)));
        env.close();
        BEAST_EXPECT (env.balance (bob, ZXC) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, ZXC) == drops(1));
    ...
    void testNoRipple()
    {
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(ZXC(10000), gw1, alice, bob);
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(ZXC(10000), gw1, gw2, alice);
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    ...
    void testInvalid()
    {
        ...
        // carol gets enough ZXC to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (ZXC (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, ZXC (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (ZXC (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (ZXC (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make ZXC and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, ZXC (100)));
            ...
            env (pay (alice, becky, ZXC (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, ZXC (100)), ter (tecNO_PERMISSION));
