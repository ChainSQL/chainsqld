...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    testPayIOU()
    {
        ...
        {
            STAmount const bobPaysZXC{env.balance(bob, ZXC) - reserve(env, 1)};
    ...
    testPayZXC()
    {
        ...
        env.close();
        BEAST_EXPECT(
            env.balance(bob, ZXC) == (reserve(env, 0) + reserve(env, 0)));
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    ...
    testPayment(FeatureBitset features)
    {
...

        {
            ...
            env(pay(becky, becky, USD(10)), path(~USD), sendmax(ZXC(10)));
            env.close();
