...
class Freeze_test : public beast::unit_test::suite
{
    ...
    void testRippleState()
    {
        ...
        env.fund(ZXC(1000), G1, alice, bob);
        env.close();
        ...
        env(offer(alice, ZXC(500), G1["USD"](100)));
        env.close();
    ...
    testGlobalFreeze()
    {
        ...
        env.fund(ZXC(12000), G1);
        env.fund(ZXC(1000), A1);
        env.fund(ZXC(20000), A2, A3, A4);
        env.close();
        ...
        env(offer(G1, ZXC(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(G1, G1["USD"](100), ZXC(10000)), txflags(tfPassive));
        env(offer(A1, ZXC(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(A2, G1["USD"](100), ZXC(10000)), txflags(tfPassive));
        env.close();
...

        {
            ...
            env(offer(A3, G1["BTC"](1), ZXC(1)));

            //    test: assets can be sold on the market
            env(offer(A4, ZXC(1), G1["BTC"](1)));
...

        {
            ...
            env(offer(A3, G1["BTC"](1), ZXC(1)), ter(tecFROZEN));

            //    test: assets can't be sold on the market
            env(offer(A4, ZXC(1), G1["BTC"](1)), ter(tecFROZEN));
        }
    ...
    testOffersWhenFrozen()
    {
        ...
        env.fund(ZXC(1000), G1, A3, A4);
        env.fund(ZXC(2000), A2);
        env.close();
        ...
        env(offer(A3, ZXC(1000), G1["USD"](1000)), txflags(tfPassive));
        env.close();

        // removal after successful payment
        //    test: make a payment with partially consuming offer
        env(pay(A2, G1, G1["USD"](1)), paths(G1["USD"]), sendmax(ZXC(1)));
        env.close();
        ...
        env(offer(A4, ZXC(999), G1["USD"](999)));
        env.close();
        ...
        env(offer(A2, G1["USD"](999), ZXC(999)));
        affected = env.meta()->getJson(0)[sfAffectedNodes.fieldName];
