...
class TxQ_test : public beast::unit_test::suite
{
    ...
    openLedgerFee(jtx::Env& env)
    {
        ...
        return fee(
            toDrops(metrics.openLedgerFeeLevel, view.fees().base).second + 1);
    }
        ...
        std::uint32_t increment)
    {
        ...
        checkMetrics(env, 0, flagMaxQueue, 0, flagPerLedger, 256, 100000000000);
        ...
        BEAST_EXPECT(fees.base == ZXCAmount{base});
        BEAST_EXPECT(fees.units == FeeUnit64{units});
        BEAST_EXPECT(fees.reserve == ZXCAmount{reserve});
        BEAST_EXPECT(fees.increment == ZXCAmount{increment});
...
public:
    ...
    testQueue()
    {
        ...
        env.fund(ZXC(50000), noripple(alice, bob, charlie, daria));
        ...
        env.fund(ZXC(50000), noripple(elmo, fred, gwen, hank));
    ...
    testTecResult()
    {
        ...
        // Create accounts
        env.fund(ZXC(50000), noripple(alice, gw));
        ...
        env(offer(alice, ZXC(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, ZXC(1000), USD(1000)), ter(terQUEUED));
    ...
    testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(ZXC(50000), noripple(alice, bob, charlie));
        ...
        env(noop(alice), seq(env.seq(alice) + 2), ter(terPRE_SEQ));
        checkMetrics(env, 1, boost::none, 3, 2, 256);
    ...
    testLastLedgerSeq()
    {
        ...
        env.fund(ZXC(1000), noripple(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(ZXC(1000), noripple(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(ZXC(1000), noripple(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
        ...
        checkMetrics(env, 5, boost::none, 3, 2, 256);
        {
            ...
            BEAST_EXPECT(aliceStat.begin()->second.feeLevel == FeeLevel64{256});
            BEAST_EXPECT(
                aliceStat.begin()->second.lastValid &&
    ...
    testZeroFeeTxn()
    {
        ...
        env.fund(ZXC(1000), noripple(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(ZXC(1000), noripple(carol));
        env.close(env.now() + 5s, 10000ms);
        ...
        for (int i = 0; i < 4; ++i)
        {
            env(noop(bob), fee(feeBob), seq(seqBob), queued);
        ...
        env(regkey(bob, alice), fee(0), seq(seqBob), queued);
        ++seqBob;
        ...
        // TANSTAAFL
        env(noop(bob), fee(ZXC(100)), seq(seqBob), ter(telINSUF_FEE_P));
    ...
    testPreclaimFailures()
    {
        ...
        env.fund(ZXC(1000), noripple(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, ZXC(-1000)), ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(ZXC(100000)), ter(terINSUF_FEE_B));
    }
    ...
    testMultiTxnPerAccount()
    {
        ...
        env(noop(alice), seq(aliceSeq + 2), fee(100), ter(terPRE_SEQ));
        checkMetrics(env, 1, initQueueMax, 4, 3, 256);
        ...
        checkMetrics(env, 8, 8, 5, 4, 513);
        {
            ...
            for (auto const& [txSeq, details] : aliceStat)
            {
                ...
                BEAST_EXPECT(
                    details.feeLevel == toFeeLevel(fee, baseFee).second);
        ...
        env(noop(alice), seq(aliceSeq), fee(aliceFee), ter(terPRE_SEQ));
        checkMetrics(env, 8, 8, 5, 4, 513);
        ...
        aliceFee =
            env.le(alice)->getFieldAmount(sfBalance).zxc().drops() - (59);
        env(noop(alice),
            seq(aliceSeq),
            fee(aliceFee),
        ...
        // Alice is broke
        env.require(balance(alice, ZXC(0)));
    ...
    testAcctTxnID()
    {
        ...
        env.fund(ZXC(50000), noripple(alice));
    ...
    testUnexpectedBalanceChange()
    {
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more ZXC, and it'll process.
        */
    ...
    testBlockers()
    {
        ...
        env.fund(ZXC(50000), noripple(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
        ...
        env(signers(alice, 2, {{bob}, {charlie}, {daria}}),
            ...
            seq(aliceSeq + 2),
            queued);
        env(regkey(alice, bob), seq(aliceSeq + 2), fee(30), queued);
    ...
    testInFlightBalance()
    {
        ...
        env.fund(ZXC(50000), noripple(alice, charlie), gw);
        ...
        //////////////////////////////////////////
        // Offer with high ZXC out and low fee doesn't block
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, ZXC(50000)), owners(alice, 0));

        // If this offer crosses, all of alice's
        // ZXC will be taken (except the reserve).
        env(offer(alice, BUX(5000), ZXC(50000)), queued);
        ...
        // has plenty of ZXC, because the offer didn't
        // cross (of course).
        ...
        //////////////////////////////////////////
        // Offer with high ZXC out and high total fee blocks later txs
        fillQueue(env, alice);
        ...
        env(offer(alice, BUX(5000), ZXC(50000)), fee(drops(100)), queued);
        ...
        //////////////////////////////////////////
        // Offer with high ZXC out and super high fee blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        env(offer(alice, BUX(5000), ZXC(50000)), fee(drops(300)), queued);
        checkMetrics(env, 1, limit * 2, limit + 1, limit, 256);
        ...
        //////////////////////////////////////////
        // Offer with low ZXC out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        // of alice's ZXC will be taken.
        env(offer(alice, BUX(50), ZXC(500)), queued);
        ...
        //////////////////////////////////////////
        // Large ZXC payment doesn't block later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, ZXC(50000)), queued);
        ...
        //////////////////////////////////////////
        // Small ZXC payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        env(pay(alice, charlie, ZXC(500)), queued);
        ...
        env.require(
            balance(alice, aliceBal - ZXC(500) - drops(20)), owners(alice, 0));
        ...
        // alice's ZXC balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        checkMetrics(env, 2, limit * 2, limit + 1, limit, 256);
        ...
        // ZXC balance, but her USD balance went to charlie.
        env.require(
        ...
        //////////////////////////////////////////
        // Large ZXC to IOU payment doesn't block later txs.

        env(offer(gw, ZXC(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire ZXC balance to charlie in the
        // form of USD.
        BEAST_EXPECT(ZXC(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)), sendmax(ZXC(60000)), queued);
        ...
        // to charlie using only a portion of her ZXC balance
        env.require(
            balance(alice, aliceBal - ZXC(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small ZXC to IOU payment allows later txs.
        ...
        // a portion of her ZXC balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > ZXC(6001));
        env(pay(alice, charlie, USD(500)), sendmax(ZXC(6000)), queued);
        ...
        // to charlie using only a portion of her ZXC balance
        env.require(
            balance(alice, aliceBal - ZXC(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    testConsequences()
    {
        ...
        {
            auto const jtx =
                env.jt(ticket::create(alice, "bob", 60), seq(1), fee(10));
            auto const pf = preflight(
                env.app(),
                env.current()->rules(),
                *jtx.stx,
...

        {
            ...
            auto const jtx = env.jt(cancelTicket, seq(1), fee(10));
            auto const pf = preflight(
                env.app(),
                env.current()->rules(),
                *jtx.stx,
    ...
    testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(ZXC(500000), noripple(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    testServerSubscribe()
    {
        ...
        env.fund(ZXC(50000), noripple(a, b, c, d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(ZXC(50000), noripple(e, f, g, h, i));
    ...
    testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(ZXC(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(ZXC(1)), seq(aliceSeq++), ter(terQUEUED));
