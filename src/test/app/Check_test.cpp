...
class Check_test : public beast::unit_test::suite
{
    ...
    testEnabled()
    {
        ...
        Account const alice{"alice"};
        {
            ...
            env(check::create(env.master, alice, ZXC(100)), ter(temDISABLED));
            env.close();

            env(check::cash(alice, checkId, ZXC(100)), ter(temDISABLED));
            env.close();
        ...
        }
        {
            ...
            uint256 const checkId1{
                getCheckIndex(env.master, env.seq(env.master))};
            env(check::create(env.master, alice, ZXC(100)));
            env.close();

            env(check::cash(alice, checkId1, ZXC(100)));
            env.close();
    ...
    testCreateValid()
    {
        ...
        STAmount const startBalance{ZXC(1000).value()};
        ...
        auto writeTwoChecks = [&env, &USD, this](
            ...
            env(check::create(from, to, ZXC(2000)));
            env.close();
        ...
        ZXCAmount const baseFeeDrops{env.current()->fees().base};
        env(check::create(alice, bob, USD(50)),
            msig(bogie, demon),
            fee(3 * baseFeeDrops));
        env.close();
    ...
    testCreateInvalid()
    {
        ...
        // Check to self.
        env(check::create(alice, alice, ZXC(10)), ter(temREDUNDANT));
        env.close();
    ...
    testCashZXC()
    {
        // Explore many of the valid ways to cash a check for ZXC.
        testcase("Cash ZXC");
        ...
        env.fund(startBalance, alice, bob);
        {
            ...
            env(check::cash(bob, chkId, ZXC(10)));
            env.close();
            env.require(
                balance(alice, startBalance - ZXC(10) - drops(baseFeeDrops)));
            env.require(
                balance(bob, startBalance + ZXC(10) - drops(baseFeeDrops)));
            ...
            env(pay(env.master, alice, ZXC(10) + drops(baseFeeDrops)));
            env(pay(bob, env.master, ZXC(10) - drops(baseFeeDrops * 2)));
            env.close();
    ...
    testCashIOU()
    {
        ...
        IOU const USD{gw["USD"]};
        {
            ...
            env.fund(ZXC(1000), gw, alice, bob);
    ...
    testCashInvalid()
    {
        ...
        env.fund(ZXC(1000), gw, alice, bob, zoe);
        ...
        env(check::create(alice, bob, ZXC(10)), expiration(env.now() + 1s));
        env.close();
        ...
        // Same set of failing cases for both IOU and ZXC check cashing.
        ...
        failingCases(chkIdX, ZXC(10));
        failingCases(chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env(check::cash(bob, chkIdU, USD(20)));
        env.close();
        env(check::cash(bob, chkIdX, check::DeliverMin(ZXC(10))));
        verifyDeliveredAmount(env, ZXC(10));

        // Try to cash an expired check.
        env(check::cash(bob, chkIdExp, ZXC(10)), ter(tecEXPIRED));
        env.close();
    ...
    testFix1623Enable()
    {
        ...
        auto testEnable = [this](
            ...
            uint256 const chkId{getCheckIndex(alice, env.seq(alice))};
            env(check::create(alice, bob, ZXC(200)));
            env.close();

            env(check::cash(bob, chkId, check::DeliverMin(ZXC(100))));
