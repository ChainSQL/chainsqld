...
public:
    void testRmFundedOffer ()
    {
        ...
        // be satisified. The first offer must be "taker gets" ZXC. Old, broken
        // would remove the first "taker gets" zxc offer, even though the offer is
    ...
    void testZXCTinyPayment ()
    {
        ...
        // 1) When we calculate the amount of input zxc needed for an offer from
        //    zxc->iou, the amount is less than 1 drop (after rounding up the float
        //    representation).
        ...
        for (auto withFix : {false, true})
        {
            ...
            // Offer at a quality poor enough so when the input zxc is calculated
    ...
    testSelfCross(bool use_partner)
    {
        ...
        BEAST_EXPECT(isOffer(env, account_to_test, ZXC(0), USD(0)));
    ...
    void
    testOfferCrossWithZXC(bool reverse_order)
    {
    ...
    void run ()
    {
        ...
        testSelfCross (true);
        testNegativeBalance ();
        testOfferCrossWithZXC (true);
        testOfferCrossWithZXC (false);
        testOfferCrossWithLimitOverride ();
        testOfferAcceptThenCancel ();
        testOfferCancelPastAndFuture ();
        testCurrencyConversionEntire ();
