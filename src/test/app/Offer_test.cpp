...
class Offer_test : public beast::unit_test::suite
{
    ...
    static auto
    zxcMinusFee (jtx::Env const& env, std::int64_t zxcAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" ZXC. Old, broken would remove the first
        // "taker gets" zxc offer, even though the offer is still funded and
        ...
        // "taker gets" must be ZXC
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), ZXC (49)));
        env (offer (carol, BTC (51), ZXC (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, ZXC (50), USD (50)));
        env (offer (carol, ZXC (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (ZXC, USD), Path (USD));
...
//      auto const offer3Seq = env.seq (alice);
    ...
    void testZXCTinyPayment (FeatureBitset features)
    {
        ...
        // Regression test for tiny zxc payments
        // In some cases, when the payment code calculates
        // the amount of zxc needed as input to an zxc->iou offer
        ...
        // 1) When we calculate the amount of input zxc needed for an offer
        //    from zxc->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (ZXC (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoRipple (FeatureBitset features)
    {
...

        {
            ...
            env.fund (ZXC (10000), alice, noripple (bob), carol, dan, gw1, gw2);
            ...
            env (offer (dan, ZXC (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (ZXC (50)), txflags (tfNoRippleDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (ZXC (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, zxcMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // Partial cross:
        {
            ...
            auto const zxcOffer2 = ZXC (500);

            env.fund (r + f + zxcOffer, bob);
            env (offer (bob, usdOffer2, zxcOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, zxcOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + zxcOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + zxcOffer2),
        ...
        // buy ZXC. If it fully crosses, we succeed.
        {
            ...
            env.fund (r + f + zxcOffer, bob, carol);
            env (offer (bob, usdOffer2, zxcOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, zxcOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + zxcOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + zxcOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + zxcOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = ZXC (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
            ...
            {features - fix1578, features | fix1578})
        {
            ...
            env (offer (alice, ZXC (500), USD (500)),
        ...
        // and add nothing on the books:
        {
            ...
            env.require (
                ...
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - ZXC (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            env.require (
                ...
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - ZXC (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), ZXC(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == ZXC (2000));
            ...
            env (offer (bob, ZXC(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == ZXC (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, ZXC(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), ZXC(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), ZXC(1001)));
            env.close();

            env (offer ("alice", USD(500), ZXC(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == ZXC (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        {
            // Alice tries an ZXC to ZXC order:
            env (offer (alice, ZXC (1000), ZXC (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), ZXC (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use ZXC as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, ZXC (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, zxcOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, zxcOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, zxcOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + ZXC(1), "carol");
        env (offer ("carol", usdOffer, zxcOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + ZXC(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, zxcOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + zxcOffer, "eve");
        env (offer ("eve", usdOffer, zxcOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + zxcOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        // old and the new behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == ZXC (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), ZXC (150000)));
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, zxcIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, zxcIssue()) ==
            bob_initial_balance -
    ...
    testOfferCrossWithZXC(bool reverse_order, FeatureBitset features)
    {
        ...
        if (reverse_order)
            env (offer (bob, USD (1), ZXC (4000)));

        env (offer (alice, ZXC (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                ZXC (10000).value ().mantissa () -
                ZXC (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (ZXC (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                ZXC (100000).value ().mantissa () -
                ZXC (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, ZXC (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, ZXC (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 ZXC
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
        ...
        // alice now has 200 more ZXC from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                ZXC (10000).value ().mantissa () +
                ZXC (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to ZXC which should fail
        // due to PartialPayment.
        env (pay (alice, alice, ZXC (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to ZXC, should succeed because
        ...
        // only 300 ZXC should be have been payed since that's all
    ...
    testCrossCurrencyStartZXC(FeatureBitset features)
    {
        ...
        env (offer (carol, ZXC (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (ZXC (333)));
    ...
    testCrossCurrencyEndZXC(FeatureBitset features)
    {
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), ZXC (500)));

        env (pay (alice, bob, ZXC (250)), sendmax (USD (333)));
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (ZXC (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, ZXC (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "ZXC";
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_zxc = ZXC (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_zxc, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, ZXC (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), ZXC (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == ZXC (350).value ().getText ()
        );
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_zxc, gw, alice, bob);
        ...
        env (offer (bob, ZXC (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), ZXC (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == ZXC (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, ZXC (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 ZXC.
        // Selling ZXC.
        // Will sell all 100 ZXC and get more USD than asked for.
        env (offer (alice, USD (100), ZXC (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_zxc = ZXC (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (ZXC(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for ZXC : USD.
        ...
        {
//acct                             fundZxc  bookAmt      preTrust  offerAmt                     tec       spentZxc    balanceUSD  offers  owners
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, ZXC(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, ZXC(  50) + f, USD(     50),      0,     1},
{"hye", ZXC(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, ZXC( 800) + f, USD(    800),      0,     1},
{"ivy", ZXC(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, ZXC(   1) + f, USD(      1),      0,     1},
{"joy", ZXC(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, ZXC(   1) + f, USD(      1),      1,     2},
{"kim", ZXC( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, ZXC( 999) + f, USD(    999),      0,     1},
{"liz", ZXC( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, ZXC( 998) + f, USD(    998),      0,     1},
{"meg", ZXC( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, ZXC( 999) + f, USD(    999),      0,     1},
{"nia", ZXC( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, ZXC( 999) + f, USD(    999),      1,     2},
{"ova", ZXC( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, ZXC( 999) + f, USD(    999),      0,     1},
{"pam", ZXC( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, ZXC(1000) + f, USD(   1000),      0,     1},
{"rae", ZXC( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, ZXC(1000) + f, USD(   1000),      0,     1},
{"sue", ZXC(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", ZXC(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, ZXC(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, ZXC(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, ZXC(   1) + f, USD(      1),      1,     1},
...
{"ted", ZXC(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, ZXC(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, ZXC( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, ZXC( 1) + 2*f, USD(      1),      1,     2},
};
        ...
        for (auto const& t : tests)
        {
            ...
            if (book)
                env (offer (gw, ZXC (book), USD (book)));
            env.close();
            ...
            // to be enough ZXC reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), ZXC (acctOffer)), ter (t.tec));
            env.close();
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == ZXC (leftover));
    ...
    testZXCDirectCross (FeatureBitset features)
    {
        ...
        // The scenario:
        //   o alice has USD but wants ZXC.
        //   o bob has ZXC but wants USD.
        auto const alicesZXC = env.balance (alice);
        auto const bobsZXC = env.balance (bob);

        env (offer (alice, zxcOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, zxcOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesZXC + zxcOffer - fee),
            balance (bob,   bobsZXC   - zxcOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), ZXC(999)));
        env (offer (bob, zxcOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == ZXC (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (ZXC(1000000), gw, alice, bob, carol);
        env.close();
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == ZXC (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == ZXC (600));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == ZXC (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        struct TestData
        {
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundZxc_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentZxc_, finalUsd_,
        ...
        {
// acct pays ZXC
//acct                           fundZxc  fundUSD   gwGets   gwPays  acctGets  acctPays                      tec         spentZxc  finalUSD  offers  owners  takerGets  takerPays
{"ann", ZXC(10) + reserve (env, 0) + 1*f, USD( 0), ZXC(10), USD( 5),  USD(10),  ZXC(10), tecINSUF_RESERVE_OFFER, ZXC( 0) + (1*f),  USD( 0),      0,     0},
{"bev", ZXC(10) + reserve (env, 1) + 1*f, USD( 0), ZXC(10), USD( 5),  USD(10),  ZXC(10),             tesSUCCESS, ZXC( 0) + (1*f),  USD( 0),      1,     1,     ZXC(10),  USD(10)},
{"cam", ZXC(10) + reserve (env, 0) + 1*f, USD( 0), ZXC(10), USD(10),  USD(10),  ZXC(10),             tesSUCCESS, ZXC(10) + (1*f),  USD(10),      0,     1},
{"deb", ZXC(10) + reserve (env, 0) + 1*f, USD( 0), ZXC(10), USD(20),  USD(10),  ZXC(10),             tesSUCCESS, ZXC(10) + (1*f),  USD(20),      0,     1},
{"eve", ZXC(10) + reserve (env, 0) + 1*f, USD( 0), ZXC(10), USD(20),  USD( 5),  ZXC( 5),             tesSUCCESS, ZXC( 5) + (1*f),  USD(10),      0,     1},
{"flo", ZXC(10) + reserve (env, 0) + 1*f, USD( 0), ZXC(10), USD(20),  USD(20),  ZXC(20),             tesSUCCESS, ZXC(10) + (1*f),  USD(20),      0,     1},
{"gay", ZXC(20) + reserve (env, 1) + 1*f, USD( 0), ZXC(10), USD(20),  USD(20),  ZXC(20),             tesSUCCESS, ZXC(10) + (1*f),  USD(20),      0,     1},
{"hye", ZXC(20) + reserve (env, 2) + 1*f, USD( 0), ZXC(10), USD(20),  USD(20),  ZXC(20),             tesSUCCESS, ZXC(10) + (1*f),  USD(20),      1,     2,     ZXC(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), ZXC( 5),  ZXC(10),  USD(10), tecINSUF_RESERVE_OFFER, ZXC(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), ZXC( 5),  ZXC(10),  USD(10),             tesSUCCESS, ZXC(  0) + (2*f),  USD(10),      1,     2,     USD(10),  ZXC(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), ZXC(10),  ZXC(10),  USD(10),             tesSUCCESS, ZXC(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), ZXC(20),  ZXC(10),  USD(10),             tesSUCCESS, ZXC(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), ZXC(40),  ZXC(10),  USD(10),             tesSUCCESS, ZXC(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), ZXC( 5),  ZXC(10),  USD(10),             tesSUCCESS, ZXC( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  ZXC( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), ZXC(10),  ZXC(10),  USD(10),             tesSUCCESS, ZXC(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  ZXC( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer(alice, USD(20), ZXC(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, ZXC(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), ZXC(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for ZXC(500), USD(5)
            env (offer(alice, USD(1), ZXC(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (ZXC(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for ZXC.  alice takes bob's offer.
            ...
            env (offer (bob, ZXC(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), ZXC(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, ZXC( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, ZXC(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell ZXC
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (ZXC(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), ZXC(1)));
            env.close();

            env (offer (deb, ZXC(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, ZXC( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, ZXC(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (ZXC(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), ZXC(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, ZXC(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, ZXC(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == ZXC (2000));
            ...
            env.require (balance (fyn, ZXC(22000)));
        ...
        env.fund (ZXC(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove ZXC from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (ZXC(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has ZXC but wants EUR.
            ...
            env (offer (ova, ZXC(2), USD(100)));
            env (offer (pat, EUR(100), ZXC(2)));
            env.close();
            ...
            env.require (balance (ova, ZXC(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == ZXC (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == ZXC (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        TestData const tests[]
        {
// acct               fundZXC    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(ZXC(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), ZXC(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, ZXC(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // ZXC reserve.
        env.require (balance (alice, ZXC(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == ZXC (250));
        }
    }
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy ZXC and sell USD.
        //  3. bob makes an offer to buy BTC and sell ZXC.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells ZXC to...
        //  o alice's first offer which takes bob's ZXC and sells USD to...
        ...
        struct Actor
        {
            ...
            PrettyAmount zxc;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (ZXC (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     ZXC(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, ZXC(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.zxc));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (ZXC(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), ZXC(4000)));
        env.close();
        ...
        env (offer (bob, ZXC(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), ZXC(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), ZXC(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    void testRCSmoketest(FeatureBitset features)
    {
        ...
        env.fund (ZXC(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env (offer (alice, ZXC(4000), gwUSD(40)));
        env.close();
