...
class Offer_test : public beast::unit_test::suite
{
        ...
        -> jtx::PrettyAmount
    {
        ...
        return drops(dropsPerZXC * zxcAmount - feeDrops);
    }
...
public:
    ...
    testCanceledOffer(FeatureBitset features)
    {
        ...
        env(offer(alice, ZXC(500), USD(100)), require(offers(alice, 1)));
        env.close();
        ...
        BEAST_EXPECT(
            isOffer(env, alice, ZXC(300), USD(100)) &&
        ...
        env(offer(alice, ZXC(222), USD(111)), require(offers(alice, 3)));
        env.close();
    ...
    void
    testZXCTinyPayment(FeatureBitset features)
    {
    ...
    testInsufficientReserve(FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env(offer(alice, zxcOffer, usdOffer), ter(tecINSUF_RESERVE_OFFER));
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env.require(
            balance("carol", reserve(env, 0) + ZXC(1)), owners("carol", 0));
        ...
        env.require(
            balance("eve", reserve(env, 1) + zxcOffer), owners("eve", 1));
    }
    ...
    {
        testcase(
            std::string("Offer Crossing with ZXC, ") +
        ...
        BEAST_EXPECT(
            ...
            to_string((ZXC(10000) - ZXC(reverse_order ? 4000 : 3000) -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            to_string((ZXC(100000) - ZXC(3000) - env.current()->fees().base * 1)
                          .zxc()));
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            to_string((ZXC(100000) + ZXC(3000) - env.current()->fees().base * 2)
                          .zxc()));
    }
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            to_string((ZXC(10000) + ZXC(500) - env.current()->fees().base * 2)
                          .zxc()));
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            to_string((ZXC(10000) + ZXC(200) - env.current()->fees().base * 2)
                          .zxc()));
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            to_string((ZXC(10000) + ZXC(200) + ZXC(300) -
    ...
    testCrossCurrencyEndZXC(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                ZXC(10000).value().mantissa() + ZXC(250).value().mantissa()));
    ...
    testSelfCrossOffer2(FeatureBitset features)
    {
        ...
        TestData const tests[]{
            // acct               fundZXC    fundUSD    fundEUR firstOfferTec
            // secondOfferTec
            {"ann",
             reserve(env, 3) + f * 4,
             USD(1000),
    ...
    testDeletedOfferIssuer(FeatureBitset features)
    {
        ...
        env.fund(ZXC(10000), alice, becky, carol, noripple(gw));
        ...
        // direct ZXC -> USD
        // direct BUX -> USD
        env(offer(becky, ZXC(2), USD(2)), txflags(tfPassive));
        ...
        env.close();
        BEAST_EXPECT(!trustLineExists(env, gw, becky, USD.currency));
        BEAST_EXPECT(isOffer(env, becky, ZXC(2), USD(2)));
        ...
        env(offer(alice, USD(2), ZXC(2)), ter(tecNO_ISSUER));
        env.close();
        env.require(offers(alice, 0));
        BEAST_EXPECT(isOffer(env, becky, ZXC(2), USD(2)));
        BEAST_EXPECT(isOffer(env, becky, BUX(3), USD(3)));
        ...
        env.close();
        BEAST_EXPECT(isOffer(env, alice, BUX(2), ZXC(2)));
        BEAST_EXPECT(isOffer(env, becky, ZXC(2), USD(2)));
    }
    ...
    testFalseAssert()
    {
        ...
        env.fund(ZXC(10000), alice);
        env.close();
        env(offer(alice, ZXC(100000000000), USD(100000000)));
        pass();
    }
