...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        { // Escrow not enabled
            ...
            env(lockup("alice", "bob", ZXC(1000), env.now() + 1s), ter(temDISABLED));
        ...
        { // Escrow enabled
            ...
            env.fund(ZXC(5000), "alice", "bob");
            env(lockup("alice", "bob", ZXC(1000), env.now() + 1s));
            env.close();

            auto const seq = env.seq("alice");

            env(condpay("alice", "bob", ZXC(1000),
    ...
    testFails()
    {
        ...
        env(condpay("alice", "carol", ZXC(1000),
        ...
        env (lockup("alice", "carol", ZXC(0),
            env.now() + 1s),                            ter(temBAD_AMOUNT));
        env (lockup("alice", "carol", ZXC(-1000),
            env.now() + 1s),                            ter(temBAD_AMOUNT));

        // Fail if neither CancelAfter nor FinishAfter are specified:
        {
            auto j1 = lockup("alice", "carol", ZXC(1), env.now() + 1s);
            j1.removeMember ("FinishAfter");
            env (j1, ter(temBAD_EXPIRATION));

            auto j2 = condpay("alice", "carol", ZXC(1), makeSlice(cb1), env.now() + 1s);
            j2.removeMember ("CancelAfter");
            env (j2, ter(temBAD_EXPIRATION));
        }

        // Fail if FinishAfter has already passed:
        env (lockup("alice", "carol", ZXC(1), env.now() - 1s), ter (tecNO_PERMISSION));

        // Both CancelAfter and FinishAfter
        env(condpay("alice", "carol", ZXC(1), makeSlice(cb1),
        ...
        { // Try to specify a condition for a non-conditional payment
            ...
            auto j = lockup("ivan", "ivan", ZXC(10), env.now() + 1s);
    ...
    testLockup()
    {
        ...
        { // Unconditional
            ...
            env.fund(ZXC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(lockup("alice", "alice", ZXC(1000), env.now() + 1s));
        ...
        { // Conditional
            ...
            env.fund(ZXC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(lockup("alice", "alice", ZXC(1000), makeSlice(cb2), env.now() + 1s));
            env.require(balance("alice", ZXC(4000) - drops(10)));
    ...
    testEscrowConditions()
    {
        ...
        { // Test cryptoconditions
            ...
            env(condpay("alice", "carol", ZXC(1000), makeSlice(cb1), T(S{1})));
        ...
        { // Test cancel when condition is present
            ...
            auto const seq = env.seq("alice");
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env(condpay("alice", "carol", ZXC(1000), makeSlice(cb2), T(S{1})));
            env.close();
            env.require(balance("alice", ZXC(4000) - drops(10)));
...

        {
            ...
            env.close();
            auto const seq = env.seq("alice");
            env(condpay("alice", "carol", ZXC(1000), makeSlice(cb3), T(S{1})));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
        ...
        { // Test empty condition during creation and
          // empty condition & fulfillment during finish
            ...
            env(condpay("alice", "carol", ZXC(1000), {}, T(S{1})), ter(temMALFORMED));
        ...
        { // Test a condition other than PreimageSha256, which
          // would require a separate amendment
            ...
            env(condpay("alice", "carol", ZXC(1000), makeSlice(cb), T(S{1})),
                ter(temMALFORMED));
        }
    }
    ...
    testMetaAndOwnership()
    {
...

        {
            ...
            env(lockup(alice, carol, ZXC(1000), env.now() + 1s));
...

        {
            ...
            env(lockup(alice, alice, ZXC(1000), env.now() + 1s, env.now() + 500s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
            ...
            env(lockup(bruce, bruce, ZXC(1000), env.now() + 1s, env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
...

        {
            ...
            env(lockup(alice, bruce, ZXC(1000), env.now() + 1s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
            env(lockup(bruce, carol, ZXC(1000), env.now() + 1s, env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
