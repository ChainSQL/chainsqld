...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (FeatureBitset features)
    {
        ...
        env.fund (ZXC (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (FeatureBitset features)
    {
        ...
        env.fund (ZXC (10000), alice, gw1, gw2);
    ...
    void testReserve(FeatureBitset features)
    {
        ...
        auto accountFundsZXC = [](ReadView const& view,
            AccountID const& id, beast::Journal j) -> ZXCAmount
        {
            return toAmount<ZXCAmount> (accountHolds (
                view, id, zxcCurrency (), zxcAccount (), fhZERO_IF_FROZEN, j));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> ZXCAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, zxcAccount (), alice, ZXC(100), env.journal);
            accountSend (sb, alice, zxcAccount (), ZXC(100), env.journal);
            BEAST_EXPECT(
                accountFundsZXC (sb, alice, env.journal) == beast::zero);
        }
    }
