...
//      static_assert (64 >= (2 * DIR_NODE_MAX), "");
        ...
        for (char b = 'A'; b <= 'D'; ++b)
        {
            for (char c = 'A'; c <= 'P'; ++c)
            {
                ...
                env.close();
                env(offer(alice, currency(50), ZXC(10)));
                env(offer(becky, ZXC(10), currency(50)));
                env.close();
            }
        }
        ...
        env.close();
        env(offer(alice, lastCurrency(50), ZXC(10)));
        env(offer(becky, ZXC(10), lastCurrency(50)));
        env.close();
