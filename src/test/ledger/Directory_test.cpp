...
struct Directory_test : public beast::unit_test::suite
{
    ...
    testDirectoryOrdering()
    {
        ...
        // the offers.
        auto book = BookDirs(*env.current(), Book({zxcIssue(), USD.issue()}));
        int count = 1;
        ...
        for (auto const& offer : book)
        {
            ...
            BEAST_EXPECT(offer->getFieldAmount(sfTakerGets) == ZXC(count));
        }
    }
    ...
    testRipd1353()
    {
        ...
        // should have no entries and be empty:
        {
            ...
            uint256 const bookBase = getBookBase({zxcIssue(), USD.issue()});
