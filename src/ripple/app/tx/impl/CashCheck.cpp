...
CashCheck::preclaim(PreclaimContext const& ctx)
{
    ...
    auto checkRet = checkAuthority(ctx, srcId, lsfPaymentAuth);
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional ZXC
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in ZXC, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += ZXCAmount{ctx.view.fees().increment};
...
CashCheck::doApply()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do ZXC to ZXC transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of ZXC sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid{zxcLiquid(psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const zxcDeliver{
                optDeliverMin
                    ? std::max(*optDeliverMin, std::min(sendMax, srcLiquid))
                    : ctx_.tx.getFieldAmount(sfAmount)};

            if (srcLiquid < zxcDeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient ZXC: "
                                 << srcLiquid.getFullText() << " < "
                                 << zxcDeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }

            if (optDeliverMin && doFix1623)
                // Set the DeliveredAmount metadata.
                ctx_.deliver(zxcDeliver);

            // The source account has enough ZXC so make the ledger change.
            if (TER const ter{
                    transferZXC(psb, srcId, account_, zxcDeliver, viewJ)};
                ter != tesSUCCESS)
            {
