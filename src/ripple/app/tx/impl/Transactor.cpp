...
Transactor::operator()()
{
    ...
    if ((terResult.ter == tecOVERSIZE) || (terResult.ter == tecKILLED) ||
        (isTecClaimHardFail (terResult.ter, view().flags())))
    {
        JLOG(j_.trace()) << "reapplying because of " << transToken(terResult.ter);
    ...
    if (applied)
    {
        ...
        if (terResult.ter == tecINVARIANT_FAILED)
        {
            ...
            // violate invariants.
            terResult.ter = ctx_.checkInvariants(terResult.ter, fee);
        }
