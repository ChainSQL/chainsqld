    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isZXC(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as ZXC or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send ZXC or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isZXC(node().issue_) ?
                        zxcAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isZXC (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isZXC(previousNode().issue_.currency) ?
                        uInAccountID : zxcAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
