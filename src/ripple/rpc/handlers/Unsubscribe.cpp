...
Json::Value doUnsubscribe (RPC::Context& context)
{
	...
	if (context.params.isMember(jss::accounts_contract))
	{
		...
		if (ids.empty())
			return rpcError(rpcACT_MALFORMED);
		context.netOps.unsubAccount(ispSub, ids, InfoSub::ACCOUNT_CONTRACT);
	}
	...
	if (context.params.isMember(jss::transaction))
	{
		...
		if (uTxId.isZero())
			return rpcError(rpcINVALID_PARAMS);
		context.netOps.unsubTransaction(ispSub, uTxId);
	}

	if (context.params.isMember(jss::owner) && context.params.isMember(jss::tablename))
	{
		auto sTableName = context.params[jss::tablename].asString();
		auto owner = ripple::parseBase58<AccountID>(context.params[jss::owner].asString());
		if (!owner)
			return rpcError(rpcINVALID_PARAMS);

		context.netOps.unsubTable(ispSub,*owner,sTableName);
	}
