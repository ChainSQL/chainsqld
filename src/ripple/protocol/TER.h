...
template <>              class CanCvtToNotTEC<TECcodes> : public std::true_type {};
...
struct STer {
	...
	STer(NotTEC ter, std::string msg = "")
	{
		this->ter = TER::fromInt(TERtoInt(ter));
		this->msg = msg;
	}

	STer(TERUnderlyingType ter, std::string msg = "")
	{
		this->ter = TER::fromInt(ter);
		this->msg = msg;
	}
