...
{
    int i;
    unsigned char temp[16], tmp_iv[16];
    ...
    if( mode == SM4_ENCRYPT )
    {
        ...
        {
            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( input[i] ^ tmp_iv[i] );

            sm4_one_round( ctx->sk, output, output );
            memcpy( tmp_iv, output, 16 );
    ...
    else /* SM4_DECRYPT */
    {
        while( length > 0 )
        {
            ...
            for( i = 0; i < 16; i++ )
                output[i] = (unsigned char)( output[i] ^ tmp_iv[i] );

            memcpy( tmp_iv, temp, 16 );
...
// mode: 1--Enc, 0--Dec
void  SM4_ECB_Opt(int mode, unsigned char *key, 
	unsigned char *input, unsigned int length, unsigned char *output)
{
	sm4_context ctx;
	...
	sm4_crypt_ecb(&ctx, mode, length, input, output);
	return;
}

// mode: 1--Enc, 0--Dec
void  SM4_CBC_Opt(int mode, unsigned char *key, unsigned char *iv, 
	unsigned char *input, unsigned int length, unsigned char *output)
{
	sm4_context ctx;
	...
	sm4_crypt_cbc(&ctx, mode, length, iv, input, output);
	return;
}
