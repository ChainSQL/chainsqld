...
public:
	...
	void OnCreateTableTx(STObject const& tx, std::shared_ptr<Ledger const> const& ledger, uint32_t time, uint256 const& chainId);
...
private:	
    ...
    bool InsertListDynamically(AccountID accountID, std::string sTableName, std::string sNameInDB, LedgerIndex seq, uint256 uHash, uint32_t time,uint256 chainId);
