...
ConfigSite::Site::Resource::Resource(std::string uri_) : uri{std::move(uri_)}
{
...
bool
ConfigSite::load(std::vector<std::string> const& siteURIs)
{
    JLOG(j_.debug()) << "Loading configured validator list sites";
...
void
ConfigSite::onRequestTimeout(std::size_t siteIdx, error_code const& ec)
{
    if (ec)
        return;
...
void
ConfigSite::onTimer(std::size_t siteIdx, error_code const& ec)
{
    if (ec)
    {
    ...
    std::lock_guard<std::mutex>& sites_lock)
{
    ...
    {
        JLOG(j_.debug()) << "Applied new validator list from "
                         << sites_[siteIdx].activeResource->uri;
    }
    else if (ListDisposition::same_sequence == disp)
    {
        JLOG(j_.debug()) << "Validator list with current sequence from "
                         << sites_[siteIdx].activeResource->uri;
    }
    else if (ListDisposition::stale == disp)
    {
        JLOG(j_.warn()) << "Stale validator list from "
                        << sites_[siteIdx].activeResource->uri;
    }
    else if (ListDisposition::untrusted == disp)
    {
        JLOG(j_.warn()) << "Untrusted validator list from "
                        << sites_[siteIdx].activeResource->uri;
    }
    else if (ListDisposition::invalid == disp)
    {
        JLOG(j_.warn()) << "Invalid validator list from "
                        << sites_[siteIdx].activeResource->uri;
    }
    else if (ListDisposition::unsupported_version == disp)
    {
        JLOG(j_.warn()) << "Unsupported version validator list from "
                        << sites_[siteIdx].activeResource->uri;
    }
    else
    {
    ...
    std::lock_guard<std::mutex>& sites_lock)
{
    ...
    else
    {
        ...
        sp = std::make_shared<detail::WorkFile>(
            resource->pUrl.path, ios_, onFetchFile);
    }
