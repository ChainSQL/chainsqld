...
void test(SleOps &s_) {
	return;
	int nTestStep = 2;
	beast::Journal j = s_.ctx().app.journal("Executive");
	AccountID userAddr = s_.ctx().tx.getAccountID(sfAccount);

	//var user = {
	//	address: "zpMZ2H58HFPB5QTycMGWSXUeF47eA8jyd4",
	//	secret : "xnnUqirFepEKzVdsoBKkMf577upwT"
	//}
	std::string sUserAddr1 = "zpMZ2H58HFPB5QTycMGWSXUeF47eA8jyd4";
	AccountID userAddr1 = parseBase58<AccountID>(sUserAddr1).get();
	std::string sTableName = "test1", sTableNameNew = "testNew1";
	bool rel;
	std::string sDetailStep;
	switch (nTestStep) {
	case 1:
	{
		sDetailStep = "payment";
		rel = (tesSUCCESS == s_.doPayment(userAddr, userAddr1, uint256(100)));
	}
	break;
	case 2:
	{
		sDetailStep = "createTable";
	...
	case 3:
	{
		sDetailStep = "dropTable";
		rel = s_.dropTable(userAddr, sTableName);
	}
	break;
	case 4:
	{
	...
	JLOG(j.warn()) << " nTestStep: " << nTestStep << " " << sDetailStep << " rel: " << rel << " >>>++++>>>";
}
