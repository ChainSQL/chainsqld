...
#ifndef PRECOMPILED_ABI_H_INCLUDE
#define PRECOMPILED_ABI_H_INCLUDE
...
template <class T>
struct ABIDynamicType<
    T,
    typename std::enable_if<
        ABIStringType<typename remove_dimension<T>::type>::value ||
...
struct Length<
    T,
    typename std::enable_if<
        ABIStaticArray<T>::value && !ABIDynamicType<T>::value>::type>
{
    enum {
        value = std::tuple_size<T>::value *
            Length<typename std::tuple_element<0, T>::type>::value
    };
};
...
{
    enum { value = Offset<T>::value + Offset<List...>::value };
};
...
private:
    ...
    void
    validOffset(std::size_t _offset)
    {
...
public:
    ...
    bytes
    serialise(const bool& _in);

    // equivalent to uint160, except for the assumed interpretation and language
    // typing. For computing the function selector, address is used. bool is
    // used.
    bytes
    serialise(const Address& _in);

    // binary type of 32 bytes
    bytes
    serialise(const string32& _in);

    // dynamic sized unicode string assumed to be UTF-8 encoded.
    bytes
    serialise(const std::string& _in);

    // static array
    template <class T, std::size_t N>
    bytes
    serialise(const std::array<T, N>& _in);
    // dynamic array
    template <class T>
    bytes
    serialise(const std::vector<T>& _in);

    template <class T>
    void
    deserialise(const T& _t, std::size_t _offset)
    {  // unsupport type
        (void)_t;
        (void)_offset;
        static_assert(ABIElementType<T>::value, "ABI not support type.");
    ...
    void
    deserialise(s256& out, std::size_t _offset);

    void
    deserialise(u256& _out, std::size_t _offset);

    void
    deserialise(bool& _out, std::size_t _offset);

    void
    deserialise(Address& _out, std::size_t _offset);

    void
    deserialise(string32& _out, std::size_t _offset);

    void
    deserialise(std::string& _out, std::size_t _offset);

    // static array
    template <class T, std::size_t N>
    void
    deserialise(std::array<T, N>& _out, std::size_t _offset);
    // dynamic array
    template <class T>
    void
    deserialise(std::vector<T>& _out, std::size_t _offset);
...
private:
    ...
    template <class T, class... U>
    void
    abiInAux(T const& _t, U const&... _u)
    {
...
public:
    template <class... T>
    bool
    abiOut(bytesConstRef _data, T&... _t)
    {
        data = _data;
        offset = 0;
        try
...
template <class T, std::size_t N>
bytes
ContractABI::serialise(const std::array<T, N>& _in)
{
    bytes offset_bytes;
    bytes content;
...
template <class T>
bytes
ContractABI::serialise(const std::vector<T>& _in)
{
    bytes offset_bytes;
    bytes content;
...
template <class T, std::size_t N>
void
ContractABI::deserialise(std::array<T, N>& _out, std::size_t _offset)
{
    for (std::size_t u = 0; u < N; ++u)
    {
        ...
        {  // dynamic type
            ...
            u256 length;
            deserialise(
                length, _offset + u * Offset<T>::value * MAX_BYTE_LENGTH);
...
template <class T>
void
ContractABI::deserialise(std::vector<T>& _out, std::size_t _offset)
{
    u256 length;
    // vector length
    ...
    for (std::size_t u = 0; u < static_cast<std::size_t>(length); ++u)
    {
        ...
        {  // dynamic type
            ...
            deserialise(
                thisEleOffset,
                _offset + u * Offset<T>::value * MAX_BYTE_LENGTH);
