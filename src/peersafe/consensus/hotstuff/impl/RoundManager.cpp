...
RoundManager::start(bool validating)
{
    ...
    // assert(stop_ == false);
    // open a new round
    boost::optional<NewRoundEvent> new_round_event =
        round_state_->ProcessCertificates(block_store_->sync_info());
    if (new_round_event)
    {
...
RoundManager::stop()
{
    ...
    JLOG(journal_.info()) << "stopped. Self is "
                          << toBase58(
                                 TokenType::NodePublic,
                                 proposal_generator_->author());
}
...
int
RoundManager::ProcessNewRoundEvent(const NewRoundEvent& new_round_event)
{
    JLOG(journal_.info())
    ...
    boost::asio::steady_timer& roundTimeoutTimer =
        round_state_->RoundTimeoutTimer();
    roundTimeoutTimer.expires_from_now(std::chrono::seconds(config_.timeout));
    roundTimeoutTimer.async_wait(std::bind(
        &RoundManager::ProcessLocalTimeout,
        this,
        std::placeholders::_1,
        new_round_event.round));

    auto const& certified_block =
        block_store_->HighestQuorumCert().certified_block();
    auto const& vote_data = block_store_->HighestQuorumCert().vote_data();
    // open a new ledger round
    if (!block_store_->state_compute()->syncState(certified_block))
    {
    ...
    if (!IsValidProposer(
            proposal_generator_->author(),
            new_round_event.round + vote_data.tc()))
    {
        JLOG(journal_.info())
            << "ProcessNewRoundEvent: New round is " << new_round_event.round
            << ", tc is " << vote_data.tc() << " and I am a voter";
        return 1;
    }

    JLOG(journal_.info()) << "ProcessNewRoundEvent: New round is "
                          << new_round_event.round << ", tc is "
                          << vote_data.tc() << " and I am the proposer";
...
boost::optional<Block>
RoundManager::GenerateProposal(const NewRoundEvent& event)
{
    boost::optional<BlockData> proposal =
        proposal_generator_->Proposal(event.round);
    if (proposal)
    {
...
void
RoundManager::GenerateThenBroadCastProposalInTimer(const NewRoundEvent& event)
{
    if (proposal_generator_->canExtract())
    {
        ...
        if (proposal)
        {
            JLOG(journal_.info())
                //<< "The author " << proposal_generator_->author()
                << "Broadcast a proposal whose round is "
                << proposal.get().block_data().round;
    ...
    {
        JLOG(journal_.debug())
            << "delay to generate a proposal for round " << event.round;
        boost::asio::steady_timer& timer =
            round_state_->GenerateProposalTimeoutTimer();
        timer.expires_from_now(
            std::chrono::milliseconds(config_.interval_extract));
        timer.async_wait(std::bind(
            &RoundManager::GenerateThenBroadCastProposal,
            this,
            std::placeholders::_1,
            event));
    }
...
/// The replica broadcasts a "timeout vote message", which includes the round
/// signature, which can be aggregated to a TimeoutCertificate. The timeout vote
/// message can be one of the following three options: 1) In case a validator
/// has previously voted in this round, it repeats the same vote and sign a
/// timeout. 2) Otherwise vote for a NIL block and sign a timeout.
void
RoundManager::ProcessLocalTimeout(
    const boost::system::error_code& ec,
    Round round)
{
    ...
    if (round != round_state_->current_round())
    {
        JLOG(journal_.error())
            ...
            << "Mismatch round: round in local timeout must be equal current "
               "round,"
...
bool
RoundManager::CheckProposal(const Block& proposal, const SyncInfo& sync_info)
{
    if (stop_)
        return false;

    JLOG(journal_.info()) << "Check a proposal: round: "
                          << proposal.block_data().round << ", seq: "
                          << proposal.block_data().getLedgerInfo().seq;

    HashValue certified_block_id =
        sync_info.HighestQuorumCert().certified_block().id;

    if (certified_block_id.isNonZero() &&
        block_store_->existsBlock(certified_block_id) == false)
    {
        ...
        block_store_->asyncExpectBlock(
            ...
            [this, certified_block_id, proposal, sync_info](
                const StateCompute::AsyncBlockErrorCode ec,
                ...
                {
                    JLOG(journal_.error())
                        << "Async block whose id is " << certified_block_id
                        << " was failure. ErrorCode is " << ec;
...
bool
RoundManager::preCheck(const Round round, const SyncInfo& sync_info)
{
    ...
    if (sync_info.hasNewerCertificate(local_sync_info))
    {
        ...
        if (const_cast<SyncInfo&>(sync_info).Verify(
                hotstuff_core_->epochState()->verifier) == false)
        {
            JLOG(journal_.error()) << "preCheck: Verifing sync_info failed";
            return false;
        }
    }
    return true;
...
int
RoundManager::preProcessProposal(
    ...
    const SyncInfo& sync_info)
{
    ...
    {
        JLOG(journal_.warn()) << "Stale proposal, current round "
    ...
    if (signature)
    {
        if (hotstuff_core_->epochState()->verifier->verifySignature(
                proposal.block_data().author(), signature.get(), proposal) ==
            false)
        {
            JLOG(journal_.warn()) << "CheckProposal: using an author "
                                  << proposal.block_data().author()
...
int
RoundManager::ProcessProposal(const Block& proposal)
{
    JLOG(journal_.info()) << "Process a proposal, round: "
                          << proposal.block_data().round << ", tc: "
                          << proposal.block_data().quorum_cert.vote_data().tc()
                          << ", seq: "
                          << proposal.block_data().getLedgerInfo().seq;
    ...
    if (IsValidProposal(proposal) == false)
    {
        JLOG(journal_.error())
            << "Proposer "
            << toBase58(TokenType::NodePublic, proposal.block_data().author())
            << " for the proposal"
    ...
    Author next_leader = proposer_election_->GetValidProposer(
        proposal_round + 1 +
        proposal.block_data().quorum_cert.vote_data().tc());
    JLOG(journal_.info())
        //<< "The author " << proposal_generator_->author()
        << "Send the vote whose round is "
        << proposal_round
...
int
RoundManager::ProcessVote(const Vote& vote, const SyncInfo& sync_info)
{
    if (stop_)
        return 1;

    Round round = vote.vote_data().proposed().round;
    JLOG(journal_.info()) << "Process a vote, round: " << round
                          << ", tc: " << vote.vote_data().tc() << ", seq: "
                          << vote.vote_data().proposed().ledger_info.seq;
...
int
RoundManager::preProcessVote(const Vote& vote, const SyncInfo& sync_info)
{
    if (EnsureRoundAndSyncUp(
            vote.vote_data().proposed().round, sync_info, vote.author()) ==
        false)
    {
        JLOG(journal_.info())
            << "Stale vote, current round " << round_state_->current_round();
        return 1;
    }
...
int
RoundManager::ProcessVote(const Vote& vote)
{
    Round round = vote.vote_data().proposed().round;
    if (vote.isTimeout() == false)
    {
        ...
        {
            JLOG(journal_.warn())
                << "Received a vote, but i am not a valid proposer for this "
                   "round "
                << (round + 1) << ", tc: " << vote.vote_data().tc()
                << ", ignore.";
            return 1;
        }
    }
    ...
    if (ret == PendingVotes::VoteReceptionResult::NewQuorumCertificate)
    {
        JLOG(journal_.info())
            //<< "The author " << proposal_generator_->author()
            << "Collected enough votes for the round "
            << vote.vote_data().proposed().round << ". A new round "
            << (round_state_->current_round() + 1) << " will open.";
    ...
    else
    {
        JLOG(journal_.warn())
            ...
            << ". Hash of ledger_info "
            << vote.ledger_info().consensus_data_hash << ". Insert result is "
            << ret;
    }
    ...
    const ripple::hotstuff::SyncInfo& sync_info)
{
    ...
    HashValue consensus_data_hash =
        epoch_change.ledger_info.ledger_info.consensus_data_hash;
    if (consensus_data_hash !=
            sync_info.HighestCommitCert()
                .ledger_info()
                .ledger_info.consensus_data_hash &&
        consensus_data_hash !=
            sync_info.HighestQuorumCert()
                .ledger_info()
                .ledger_info.consensus_data_hash)
    {
        JLOG(journal_.error()) << "Mismatch consensus hash in betwwen HQC and "
                                  "HCC in checkEpochChange.";
        assert(false);
        return false;
    }

    if (hotstuff_core_->epochState()->verifier->checkVotingPower(
            epoch_change.ledger_info.signatures) == false)
    {
        JLOG(journal_.error())
    ...
    if (EnsureRoundAndSyncUp(
            epoch_change.round + 1, sync_info, epoch_change.author) == false)
    {
        JLOG(journal_.error())
            << "Stale epoch change, current round "
            << round_state_->current_round() << " in checkEpochChange";
        return false;
    }
...
bool
RoundManager::ExecuteAndVote(const Block& proposal, Vote& vote)
{
...
bool
RoundManager::EnsureRoundAndSyncUp(
    Round round,
    const SyncInfo& sync_info,
    const Author& author)
{
    ...
    {
        JLOG(journal_.error())
            << "After sync, round " << round << " dosen't match local round "
            << current_round;
...
int
RoundManager::SyncUp(const SyncInfo& sync_info, const Author& author)
{
    SyncInfo local_sync_info = block_store_->sync_info();
    ...
    if (sync_info.hasNewerCertificate(local_sync_info))
    {
        JLOG(journal_.info()) << "local sync info is stale than remote";

        if (const_cast<SyncInfo&>(sync_info).Verify(
                hotstuff_core_->epochState()->verifier) == false)
        {
            JLOG(journal_.error()) << "Verifing sync_info failed";
            return 1;
        }
...
int
RoundManager::ProcessCertificates()
{
    SyncInfo sync_info = block_store_->sync_info();
    boost::optional<NewRoundEvent> new_round_event =
        round_state_->ProcessCertificates(sync_info);
    if (new_round_event)
    {
...
int
RoundManager::NewTCAggregated(const TimeoutCertificate& timeoutCert)
{
...
void
RoundManager::UseNilBlockProcessLocalTimeout(const Round& round)
{
    Vote timeout_vote;
    auto const& sync_info = block_store_->sync_info();
    auto const& hqc = sync_info.HighestQuorumCert();

    if (round_state_->send_vote() &&
        round_state_->send_vote()->vote_data().parent().round > 0)
    {
        timeout_vote = round_state_->send_vote().get();
        if (timeout_vote.vote_data().tc() != hqc.vote_data().tc() + 1)
        {
            JLOG(journal_.info())
                << "reset time out vote tc to " << hqc.vote_data().tc() + 1;
            hotstuff_core_->resetVoteTimeoutCount(
                timeout_vote, hqc.vote_data().tc() + 1);
        }
    }
    else
    {
        // Didn't vote in this round yet, generate a backup vote
        // generates a dummy block and inc timeout count
        boost::optional<Block> nil_block =
            proposal_generator_->GenerateNilBlock(round);
        if (nil_block)
        {
            assert(nil_block->block_data().block_type == BlockData::NilBlock);
            if (ExecuteAndVote(nil_block.get(), timeout_vote) == false)
            {
                JLOG(journal_.error()) << "Execute and vote failed for the nil "
                                          "block whose round is "
            ...
            JLOG(journal_.info())
                ...
                << nil_block.get().block_data().round << " in local timeout.";
        }
        else
        {
            JLOG(journal_.error()) << "Generate NilBlock whose round is "
                                   << nil_block.get().block_data().round
...
void
RoundManager::NotUseNilBlockProcessLocalTimeout(const Round& round)
{
    Vote timeout_vote;
    if (round_state_->send_vote())
    {
        ...
        JLOG(journal_.warn()) << "broadcast timout vote: "
    ...
    else
    {
        ...
        Author expect_next_author = proposer_election_->GetValidProposer(
            round + round_state_->getOffsetRound());
        JLOG(journal_.warn()) << "The next leader may be exception, "
                                 "so we should pick up a next leader for "
                                 "generating a proposal (round "
                              << round << ").";
        JLOG(journal_.warn())
...
bool
RoundManager::IsValidProposer(const Vote& vote)
{
    bool isValid = false;
    ...
    isValid = IsValidProposer(
        proposal_generator_->author(), next_round + vote.vote_data().tc());

    JLOG(journal_.debug())
        << "Is valid proposer for the vote? " << isValid
        << ". The Author of the vote is " << vote.author()
        << ", The round of the vote is "
...
bool
RoundManager::IsValidProposer(const Author& author, const Round& round)
{
    return proposer_election_->IsValidProposer(author, round);
}

bool
RoundManager::IsValidProposal(const Block& proposal)
{
    bool isValid = false;
    if (config_.disable_nil_block)
    {
        ...
        // assert(offsetRound);
        Author expect_autor =
            proposer_election_->GetValidProposer(propsal_round + offsetRound);
        isValid = expect_autor == proposal.block_data().author();

        JLOG(journal_.info())
            << "Is valid propsal? " << isValid
            << ". The round of the propsal is " << propsal_round
            << "and the author of the proposal is "
            << proposal.block_data().author() << ". The offset round is "
            << offsetRound << ". The expected author is " << expect_autor;
    }
    else
    {
...
bool
RoundManager::ReceivedProposedBlock(const HashValue& proposed_id)
{
    ExecutedBlock block;
...
void
RoundManager::AddVoteToCache(const Vote& vote)
{
    JLOG(journal_.info()) << "Received a vote but it's proposal hasn't been "
                             "received now, cache it."
                          << "Vote round is "
                          << vote.vote_data().proposed().round
                          << " and author is "
                          << toBase58(TokenType::NodePublic, vote.author());
...
void
RoundManager::HandleCacheVotes(const HashValue& id)
{
...
bool
RoundManager::unsafetyExpectBlock(
    const HashValue& block_id,
    ExecutedBlock& executed_block)
{
