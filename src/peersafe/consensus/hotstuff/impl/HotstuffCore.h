...
struct SafetyData
{
    ...
    SafetyData()
        : epoch(0), last_voted_round(0), preferred_round(0), last_vote()
    {
    }
};
...
public:
    ...
    void
    Initialize(Epoch epoch, Round round);

    Block
    SignProposal(const BlockData& proposal);
    bool
    ConstructAndSignVote(const ExecutedBlock& executed_block, Vote& vote);
    void
    resetVoteTimeoutCount(Vote& vote, std::size_t tc);
...
private:
    bool
    VerifyAuthor(const Author& author);
    bool
    VerifyEpoch(const Epoch epoch);
    bool
    VerifyQC(const QuorumCertificate& qc);
    bool
    VerifyAndUpdatePreferredRound(const QuorumCertificate& qc);
    bool
    VerifyAndUpdateLastVoteRound(Round round);
    std::pair<bool, VoteData>
    ExtensionCheck(const ExecutedBlock& executed_block);
    LedgerInfoWithSignatures::LedgerInfo
    ConstructLedgerInfo(const Block& proposed_block);
