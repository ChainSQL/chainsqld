...
PeerManagerImpl::PeerManagerImpl(Schema& app)
    : app_(app), journal_(app_.journal("PeerManager"))
{
}
...
void
PeerManagerImpl::add(std::shared_ptr<PeerImp> const& peer)
{
    std::lock_guard lock{mutex_};
...
void
PeerManagerImpl::remove(std::vector<PublicKey> const& vecPubs)
{
    std::lock_guard lock{mutex_};
    auto it = ids_.begin();
    while (it != ids_.end())
    {
        ...
        if (sp->publicValidate_ &&
            std::find(vecPubs.begin(), vecPubs.end(), *sp->publicValidate_) !=
                vecPubs.end())
        {
...
std::shared_ptr<Peer>
PeerManagerImpl::findPeerByValPublicKey(PublicKey const& pubKey)
{
    std::lock_guard<decltype(mutex_)> lock(mutex_);
    for (auto const& e : ids_)
    {
...
void
PeerManagerImpl::send(protocol::TMConsensus& m)
{
...
void
PeerManagerImpl::relay(protocol::TMConsensus& m, uint256 const& uid)
{
